...
def create_train_op(loss, hparams):
  train_op = tf.contrib.layers.optimize_loss(loss=loss, global_step=tf.train.get_global_step(), learning_rate=hparams.learning_rate, clip_gradients=10.0, optimizer=hparams.optimizer)
  return train_op
...
def create_model_fn(hparams, model_impl):
  def model_fn(features, targets, mode):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      for i in range(9):
        distractor, distractor_len = get_id_feature(features, "distractor_{}".format(i), "distractor_{}_len".format(i), hparams.max_utterance_len)
      ...
      probs, loss = model_impl(hparams, mode, tf.concat(0, all_contexts), tf.concat(0, all_context_lens), tf.concat(0, all_utterances), tf.concat(0, all_utterance_lens), tf.concat(0, all_targets))
